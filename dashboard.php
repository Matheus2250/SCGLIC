...
// Definir filtro por ano usando nova estrutura
if (isAnoHistorico($ano_selecionado)) {
    $where_ano = "1=1 AND"; // Para tabelas históricas específicas não precisa filtro de ano
} else {
    // Para anos atuais, filtrar por importações do ano específico
    $importacoes_ano_sql = "SELECT id FROM pca_importacoes WHERE ano_pca = $ano_selecionado";
    $importacoes_result = $pdo->query($importacoes_ano_sql);
    $importacoes_ids = [];
    while ($row = $importacoes_result->fetch()) {
...
// Buscar estatísticas para os cards e gráficos - usar nova estrutura
if (isAnoHistorico($ano_selecionado)) {
    // Para anos históricos, buscar dados da tabela específica
    $stats = [
    ...
    // Para anos atuais, buscar dados específicos das importações do ano
    if (!empty($importacoes_ids)) {
    ...
    <div class="dashboard-container">
        ...
        <div class="main-content">
            ...
            <div id="importar-pca" class="content-section <?php echo $secao_ativa === 'importar-pca' ? 'active' : ''; ?>">
                ...
                <?php if ($eh_historico): ?>
                    <!-- Formulário para importação de dados históricos -->
                    <div style="background: #fff3cd; border-left: 4px solid #ffc107; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
                        <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 10px;">
                            ...
                            <strong style="color: #856404;">Importação de Dados Históricos</strong>
                        </div>
                        <p style="color: #856404; margin: 0 0 15px 0; line-height: 1.5;">
                            Você está importando dados históricos do ano <?php echo $ano_selecionado; ?>. 
                            Estes dados serão armazenados como registros históricos (somente leitura após importação).
                        </p>
                        
                        <div class="upload-card" style="background: white; border: 2px solid #ffc107; margin-top: 15px;">
                            <h3 style="color: #856404;">Importar PCA Histórico - Ano <?php echo $ano_selecionado; ?></h3>
                            ...
                            <form action="process.php" method="POST" enctype="multipart/form-data">
                                ...
                                <br><br>
                                <button type="submit" class="btn-primary" style="background: #ffc107; border-color: #ffc107; color: #212529;">
                                    <i data-lucide="upload"></i> Importar Dados Históricos
                                </button>
                                <small style="display: block; margin-top: 10px; color: #856404;">
                                    ⚠️ Dados históricos são inseridos como registros permanentes
                                </small>
                            </form>
                        </div>
                        
                        <div style="margin-top: 15px;">
                            <a href="?secao=importar-pca&ano=2025" style="background: #1e3c72; color: white; padding: 10px 15px; border-radius: 8px; text-decoration: none; font-weight: 600;">
                                <i data-lucide="calendar-plus"></i> Importar Dados Atuais (2025)
                            </a>
                        </div>
                    </div>
                <?php else: ?>
