...
foreach ($impactos as $imp_value => $imp_label) {
    ...
    for ($prob = 1; $prob <= 5; $prob++) {
        $count = count(array_filter($riscos, fn($r) => $r['probabilidade'] == $prob && $r['impacto'] == $imp_value));
        $nivel = '';
...
foreach ($riscos as $risco) {
    $html .= '<tr>
        ...
        <td style="text-align: center;">' . $risco['probabilidade'] . ' x ' . $risco['impacto'] . '</td>
