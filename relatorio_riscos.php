...
// Buscar riscos do mês
$sql_riscos = "SELECT r.*, p.titulo_contratacao, p.area_requisitante, p.valor_total_contratacao
               FROM pca_riscos r
               LEFT JOIN pca_dados p ON r.numero_dfd = p.numero_dfd
               WHERE r.mes_relatorio = ?
               GROUP BY r.id
               ORDER BY r.nivel_risco DESC, r.numero_dfd";
...
<body>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td style="text-align: center;">
                                    <strong><?php echo $risco['probabilidade'] . ' x ' . $risco['impacto']; ?></strong>
                                </td>
                                <td style="text-align: center;">
    ...
    <script>
        ...
        function editarRisco(id) {
            fetch('process_risco.php?acao=buscar&id=' + id)
                ...
                .then(data => {
                    if (data.success && data.data) {
                        ...
                        document.getElementById('demanda').value = risco.demanda;
                        ...
                        document.getElementById('responsavel_preventiva').value = risco.responsavel_preventiva || '';
                        document.getElementById('acao_contingencia').value = risco.acao_contingencia || '';
                        document.getElementById('responsavel_contingencia').value = risco.responsavel_contingencia || '';
                        
                        // Selecionar probabilidade e impacto
                        selectedProbability = parseInt(risco.probabilidade);
                        selectedImpact = parseInt(risco.impacto);
        ...
        function excluirRisco(id) {
            if (confirm('Tem certeza que deseja excluir este risco?')) {
                window.location.href = 'process_risco.php?acao=excluir&id=' + id;
            }
        }
        ...
        function exportarRelatorio() {
            ...
            window.open('exportar_relatorio_riscos.php?mes=' + mes, '_blank');
        }
        ...
        // Click nas células da matriz para filtrar
        document.querySelectorAll('.risk-cell').forEach(cell => {
            cell.addEventListener('click', function() {
                ...
                // TODO: Implementar filtro por probabilidade e impacto
                console.log('Filtrar por P:', prob, 'I:', imp);
            });
        });
    </script>
</body>
